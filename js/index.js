...
let data = {
  ...
  forecastPictures: document.querySelectorAll(
    "[id^='weather-condition-icon-day-']"
  ),
...
function refreshDisplay(response) {
  ...
  data.current.weatherDetails.rain.innerHTML =
  ...
  data.current.weatherDetails.windDirection.innerHTML = WindDir(
  ...
  for (let i = 0; i < numberOfDays; i++) {
    ...
    data.forecastPictures[
      i
    ].innerHTML = `<img src="${response.forecast.forecastday[i].day.condition.icon}" alt="">`;
    data.conditions[i].innerHTML =
